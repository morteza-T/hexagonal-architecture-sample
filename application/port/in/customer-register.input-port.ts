...
export class CustomerRegisterInputPort implements CustomerRegisterUseCase {
    constructor(
        private customerDataOutPutPort: CustomerDataOutPutPort,
    ){}
    async register(registerRequestDTO: RegisterRequestDTO) {
        const duplicatedCustomer = await this.customerDataOutPutPort.findOne( {phoneNumber: registerRequestDTO.phoneNumber, email: registerRequestDTO.email})
        if(duplicatedCustomer) throw Error("customer has existed")
        const customer = await this.customerDataOutPutPort.register(registerRequestDTO)
        return customer
    }
}